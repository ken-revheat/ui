...
/**
 * The stylesheet's own regression guard.
 *
 ...
 * the whole point — the v2.1.0 defect was not a missing file, it was a
 * `.rh-rail` block that simply never said `position`.
 *
 ...
 *    no error. Every test below therefore sets the viewport (and media type)
 *    FIRST and mounts fresh elements after, and `afterEach` tears the mounted
 *    tree down. Never hoist a `mount()` above a viewport change.
...
interface HappyDOMControl {
  ...
  settings: { device: { mediaType: string } };
}
...
describe("styles.css — the rail is pinned to the viewport", () => {
  ...
  afterEach(() => {
    ...
    happyDOM.settings.device.mediaType = "screen";
  });

  /** Mounts `.rh-shell > <aside class=…>` and returns both. Call AFTER the
   *  viewport/media type for this test is in place — see the file docstring. */
  ...
  it("sticks the in-flow rail to the top of the viewport, full screen height", () => {
    ...
    // A DEFINITE height, not max-height: `.rh-row--all`'s `margin-top: auto`
    // pins the All-products row and the account block to the rail's bottom
    // edge, and that edge has to be the bottom of the SCREEN.
    //
  ...
  it("overrides the height with 100dvh for collapsing-toolbar viewports", () => {
    ...
    //
    // It matters at 900–1024px (iPad landscape, and any mobile browser whose
    // toolbar collapses): `100vh` there is the LARGE viewport, taller than
    // what is actually on screen, which pushes the account block off the
    // bottom — and `overflow-y: auto` cannot rescue it, because the overflow
    // is outside the scrollport, not inside it.
    //
  ...
  it("leaves the drawer panel fixed — it is a .rh-rail too", () => {
    ...
    // — would fight that. `not.toBe("768px")` used to stand here and passed
    // for any wrong value, including the viewport-height one it meant to
    // exclude once the default viewport changed.
  ...
  it("prints the content only — no rail, no banner, no gutter", () => {
    happyDOM.settings.device.mediaType = "print";
    const { shell, el } = mount("rh-rail");
    const banner = document.createElement("header");
    banner.className = "rh-banner";
    shell.appendChild(banner);
    expect(getComputedStyle(el).display).toBe("none");
    expect(getComputedStyle(banner).display).toBe("none");
    // Chrome is hidden, so the column it occupied has to go too.
    expect(getComputedStyle(shell).gridTemplateColumns).toBe("1fr");
  });
});
