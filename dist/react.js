...
function StrokeIcon({ d, className }) {
    return (_jsx("svg", { className: className, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": true, children: d.map((path, i) => (_jsx("path", { d: path }, i))) }));
}
...
function AccountMenu({ identity, isStaff, adminHref, onSignOut }) {
    ...
    return (_jsxs("div", { className: "rh-rail__account-menu", ref: rootRef, children: [open && (_jsxs("div", { className: "rh-rail__menu", id: menuId, role: "menu", ref: menuRef, onKeyDown: onMenuKeyDown, children: [items.map((item) => (_jsx("a", { className: "rh-rail__menu-item", role: "menuitem", tabIndex: -1, href: item.href, children: item.label }, item.key))), onSignOut ? (_jsx("button", { type: "button", className: "rh-rail__menu-item rh-rail__menu-item--signout", role: "menuitem", tabIndex: -1, onClick: onSignOut, children: "Sign out" })) : (_jsx("a", { className: "rh-rail__menu-item rh-rail__menu-item--signout", role: "menuitem", tabIndex: -1, href: signOutHref, children: "Sign out" }))] })), _jsxs("button", { type: "button", className: "rh-rail__account-trigger", ref: triggerRef, "aria-haspopup": "menu", "aria-expanded": open, "aria-controls": open ? menuId : undefined, onClick: () => (open ? setOpen(false) : openAndFocusFirst()), onKeyDown: (e) => {
                ...
                }, children: [_jsx("span", { className: "rh-rail__avatar", "aria-hidden": true, children: initial }), _jsxs("span", { className: "rh-rail__account-meta", children: [_jsx("span", { className: "rh-rail__email", children: identity.email }), identity.roleLabel && _jsx("span", { className: "rh-rail__role", children: identity.roleLabel })] }), _jsx(StrokeIcon, { className: "rh-rail__caret", d: CHEVRON_ICON })] })] }));
}
/* ------------------------------------------------------------------ */
/* drawer                                                              */
/* ------------------------------------------------------------------ */
function Drawer({ id, closing, onClose, children, }) {
    ...
    const tree = (_jsxs("div", { className: `rh-drawer${closing ? " rh-drawer--closing" : ""}`, children: [_jsx("div", { className: "rh-drawer__scrim", onClick: onClose, "aria-hidden": true }), _jsxs("div", { className: "rh-drawer__panel rh-rail rh-rail--drawer", id: id, role: "dialog", "aria-modal": closing ? undefined : "true", "aria-label": "RevHeat products", ref: panelRef, 
                ...
                tabIndex: -1, children: [_jsx("button", { type: "button", className: "rh-drawer__close", "aria-label": "Close product menu", onClick: onClose, children: _jsx(StrokeIcon, { className: "rh-drawer__close-glyph", d: CLOSE_ICON }) }), children] })] }));
